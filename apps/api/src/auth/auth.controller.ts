...
export class AuthController {
    ...
    @Post('register')
    async register(
        @Body() dto: RegisterDto, @Res({ passthrough: true }) res: Response) {
    ...
    async refresh(@Req() req: Request, @Res({ passthrough: true }) res: Response) {
        const refreshToken = req.cookies[COOKIE_KEYS.REFRESH_TOKEN] as string | undefined;
        if (!refreshToken) {
    ...
    async googleCallback(@Query('code') code: string, @Res() res: Response) {
        ...
        this.authService.setTokenCookies(res, tokens);
        res.redirect(`${this.configService.get<string>('WEB_URL')}/dashboard`);
    }
